...
Elevador elevadorA = {1, {true, false, false, false, false}, "A"};
Elevador elevadorB = {1, {true, false, false, false, false}, "B"};
...
byte rowPins[ROWS] = {9, 8}; // filas del keypad
...
int pisoSeleccionado = -1;     // nivel 0 a 4
...
void loop() {
  ...
  if (key != NO_KEY) {
    ...
    else if (elevadorSeleccionado != 0 && key >= '0' && key <= '4') {
...
void simularMovimiento(Elevador* elevador, int nivelDestino) {
  if (nivelDestino < 0 || nivelDestino > 4) return;
